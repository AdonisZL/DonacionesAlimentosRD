...
- **OE3**: Emparejamiento semiautomático (motor determinista FEFO + PostGIS + restricciones) asistido por IA (Gemini) con validación humana obligatoria.
...
- **RF-01** (Alta): Registro de donantes: (a) Formalizado (RNC, Razón Social) y (b) Especial/Agrícola (Cédula/Pasaporte). Ambos: correo, contraseña cifrada, dirección y coordenadas.
...
- **RF-17** (Alta): Búsqueda de receptores compatibles con PostGIS (radio configurable).
- **RF-18** (Alta): Justificación narrativa asistida por Gemini (temperatura 0.0, sin alterar el resultado determinista).
...
| RNF-12 | bcrypt coste ≥ 12; RNC/cédulas con RBAC + cifrado de disco |
...
**Incluye**: registro/auth, inventario FEFO, emparejamiento por reglas, reportes, seguridad/RBAC.
